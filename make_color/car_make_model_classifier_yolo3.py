...
np.random.seed(42)
COLORS_make = np.random.randint(0, 255, size=(len(LABELS), 3),
                                dtype="uint8")
...
make_net = cv2.dnn.readNetFromDarknet(configPath, weightsPath)


def predict_car_make(classifier, net, COLORS, filename):
...
# if len(idxs) > 0:
    ...
    # for i in idxs.flatten():
        ...
        # # draw a bounding box rectangle and label on the image
        # color = [int(c) for c in COLORS_make[classIDs[i]]]
        # if classIDs[i] == 2:
            # start = time.time()
            # result = car_make_classifier.predict(
                # image[max(y, 0):y + h, max(x, 0):x + w])
            # end = time.time()
            # # show timing information on MobileNet classifier
            # print("[INFO] classifier took {:.6f} seconds".format(end - start))
