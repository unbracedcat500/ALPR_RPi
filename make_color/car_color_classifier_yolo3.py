...
import cv2
import os
import classifierColor

# construct the argument parse and parse the arguments
# ap = argparse.ArgumentParser()
# ap.add_argument("-i", "--image", required=True,  # default= 'car2.jpg',
...
car_color_classifier = classifierColor.Classifier()
...
color_net = cv2.dnn.readNetFromDarknet(configPath, weightsPath)

# load our input image and grab its spatial dimensions


def predict_car_color(classifier, net, COLORS, filename):
    ...
    if len(idxs) > 0:
        ...
        for i in idxs.flatten():
            ...
            if classIDs[i] == 2:
                ...
                text = "{}: {:.4f}".format(
                    result[0]['color'], float(result[0]['prob']))
...
#         # draw a bounding box rectangle and label on the image
#         color = [int(c) for c in COLORS_color_classifier[classIDs[i]]]
#         if classIDs[i] == 2:
#             start = time.time()
